...
The processor currently supports the following ISA features:
- RV32I base integer instruction set, including arithmetic, logical, branch, and memory operations.
- Load and store instructions with word-level addressing.
- Control and status register (CSR) instructions, supporting read and write access to implemented CSRs such as mcycle and minstret.

### Near-Term Planned Features
Features targeted for the next stage of development include:
- Additional standard CSRs and related instructions.
...
- **CSR File** – holds control and status registers, including performance counters and machine state registers.
...
The current design includes a baseline CSR subsystem supporting read and write access through the standard RISC-V CSR instruction set (CSRRW, CSRRS, CSRRC, and their immediate variants).

The CSR specification is maintained in [`csr_spec.xlsx`](spreadsheets/csr_spec.xlsx).
This spreadsheet defines:
- CSR addresses and names.
- Access permissions (read-only, write-only, or read/write).
- Reset values and behavioral notes.

Implemented CSRs include:
- mcycle – Cycle counter that increments every clock cycle.
- minstret – Counter that increments for each retired instruction.
- mstatus – Basic machine status register containing global enable bits and privilege-related fields.
    - Note: As of now, this iso= only used for testing

Additional CSRs and functionality will be introduced as needed as the design continues to expand.
