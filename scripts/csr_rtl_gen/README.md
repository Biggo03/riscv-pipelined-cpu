...
The flow is driven by [`csr_rtl_generator.py`](csr_rtl_generator.py), which parses the spreadsheet and produces structured outputs used throughout the project.
...
**./csr_rtl_generator.py -h**

A typical command using all major options is:

**./csr_rtl_generator.py <spreadsheet_path> <rtl_output_dir> <tb_output_dir> <macro_output_dir>**
...
3. Executes `csr_rtl_generator.py` with these paths.
