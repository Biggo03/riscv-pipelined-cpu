...
module csr_reg_file (
    ...
    always_ff @(posedge clk_i) begin : csr_write_ff
        if (reset_i) begin
            ...
            unique case (csr_addr_i)
                `MCYCLE_ADDR   : mcycle_q    <= mcycle_next;
                `MCYCLEH_ADDR  : mcycleh_q   <= mcycleh_next;
                `MINSTRET_ADDR : minstret_q  <= minstret_next;
                `MINSTRETH_ADDR: minstreth_q <= minstreth_next;
            endcase
        end
    end
    ...
    always_comb begin : csr_read_comb
        unique case (csr_addr_i)
            `MCYCLE_ADDR   : csr_rdata_o = (csr_waddr_i == csr_rdata_i) ? csr_wdata_i : mcycle_q;
            `MCYCLEH_ADDR  : csr_rdata_o = (csr_waddr_i == csr_rdata_i) ? csr_wdata_i : mcycleh_q;
            `MINSTRET_ADDR : csr_rdata_o = (csr_waddr_i == csr_rdata_i) ? csr_wdata_i : minstret_q;
            `MINSTRETH_ADDR: csr_rdata_o = (csr_waddr_i == csr_rdata_i) ? csr_wdata_i : minstreth_q;
